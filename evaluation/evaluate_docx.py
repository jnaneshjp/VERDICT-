"""Evaluate the ZIP / DOCX prototype against truth (P1): baseline vs the DOCX ML ranker.
...
Usage: python evaluation/evaluate_docx.py            (docx01-docx10, both rankers)
...
def evaluate_case(case: str, ranker: str = "baseline") -> dict:
    ...
    report = analyze_image(image_path, case, ranker)
    ...
    for art in zip_artifacts:
        ...
        row = {"anchor_block": art["anchor_block"], "state": art["state"], "type": art["type"],
               ...
               "ranker_used": art["ranker_used"], "assembly": art["assembly"],
...
def totals(cases: list, ranker: str) -> dict:
    ...
    return {
        "cases": len(cases),
        "ranker": ranker,
        "ranker_used": dict(Counter(r["ranker_used"] for r in rows)),
        "artifacts": len(rows),
...
def main() -> None:
    ...
    parser.add_argument("--ranker", nargs="+", default=["baseline", "ml"], choices=["baseline", "ml"])
    args = parser.parse_args()
    result = {"cases": args.case, "rankers": {}}
    for ranker in args.ranker:
        cases = [evaluate_case(case, ranker) for case in args.case]
        for case in cases:
            for r in case["artifacts"]:
                print(f"[{ranker}] {case['case_id']}  blk {r['anchor_block']:>3}  {r['state']:<8} {r['type']:<5} "
                      f"{(r['truth_file'] or '?'):<38} {r['damage_class'] or '?':<22} "
                      f"exact={r['exact']} prefix_ok={r['prefix_match']} attempts={r['attempts']} "
                      f"used={r['ranker_used']}")
        result["rankers"][ranker] = {"totals": totals(cases, ranker), "per_case": cases}
        print(f"[{ranker}] {json.dumps(result['rankers'][ranker]['totals'])}")
